...
export function getMyTeam() {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.get(`${URL}/Team/MyTeam`, {
        headers
      });
      console.log(response);
      dispatch(slice.actions.getMyTeamSuccess(response.data.data));
    } catch (error) {
...
export function getProfile() {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios1.get('/api/user/profile');
...
export function getUserList() {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios1.get('/api/user/manage-users');
